...
export async function fetchProperties(
  ...
  if (USE_MOCKS) {
    ...
    if (city !== "All") rows = rows.filter((r) => r.city.toLowerCase() === String(city).toLowerCase());
  ...
  const rows: PropertyRow[] = props.map((p) => ({
    ...
    name: p.name,
    address: p.address || "",
    city: "", // TODO: supply when backend provides
    state: "", // TODO: supply when backend provides
  ...
  if (city !== "All") filtered = filtered.filter((r) => r.city.toLowerCase() === String(city).toLowerCase());
