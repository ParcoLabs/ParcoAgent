...
async function main() {
  ...
  const r1 = await prisma.request.create({
    data: { title: "Kitchen sink leak in Unit 3A", description: "Tenant reports slow leak", propertyId: pine.id, status: "OPEN", reporterId: manager.id },
  });

  const r2 = await prisma.request.create({
    data: { title: "AC not cooling, Unit 1B", description: "Thermostat reads 80F", propertyId: oak.id, status: "OPEN", reporterId: manager.id },
  });
